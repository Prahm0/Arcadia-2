...
export default function ConstellationCard({ card, preview = false, featured = false }: { card: SkyCard; preview?: boolean; featured?: boolean }) {
  ...
  return <article className={styles.card} data-preview={preview}>
    <div className={styles.cardTop}><span>{definition.family}</span><span style={{ color: definition.colour }}>{featured ? "Featured" : card.earnedAt && !card.seen ? "New ✦" : "✦"}</span></div>
