...
export default function ScheduleDemo() {
  ...
  return (
    <section
      ...
      className="bg-black py-[120px] text-white lg:py-[160px]"
    >
      <Container>
        <div className="grid grid-cols-12 gap-x-6">
          <div className="col-span-12 lg:col-span-8">
            <FadeIn>
              <SectionLabel>Adaptive schedule</SectionLabel>
            </FadeIn>
            <RevealText
              id="schedule-heading"
              as="h2"
              lines={["A schedule that", "moves with you."]}
          ...
          <div className="col-span-12 mt-8 lg:col-span-4 lg:mt-0 lg:self-end">
            <FadeIn delay={0.2}>
              <p className="type-body-lg max-w-[420px] text-white/60">
                Training moves. Arcadia moves everything that depends on it, then tells you what changed.
              </p>
            </FadeIn>
          </div>
        </div>
        ...
        <FadeIn delay={0.2} y={32} duration={1} amount={0.2} className="mt-16 lg:mt-20">
          <div
            ref={sectionRef}
            className="overflow-hidden rounded-[16px] border border-white/10 bg-white text-ui-text shadow-[0_40px_120px_rgba(0,0,0,0.5)]"
          >
            <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 border-b border-ui-border px-4 py-3 sm:px-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="rounded-[6px] px-2 py-1 text-[12px] text-ui-muted transition-colors hover:bg-ui-surface hover:text-ui-text disabled:opacity-40 disabled:hover:bg-transparent"
                >
                  Replay
                </button>
              </div>
      ...
      <AnimatePresence mode="wait" initial={false}>
        {showBanner && (
          ...
          >
            ...
            <span className="hidden text-ui-muted sm:inline">Calendar update</span>
            <span className="hidden sm:inline">Basketball training moved to 5:30 PM.</span>
            <span className="sm:hidden">Training → 5:30 PM</span>
          </motion.p>
        )}
        {updated && (
          <motion.p
      ...
      <div className="flex h-9 items-end border-b border-ui-border pb-2 pl-14">
        {DAYS.map((d) => (
          <div key={d} className="flex-1 px-3 text-[13px]">
            ...
            <span className="tabular text-ui-muted">{DAY_LABELS[d].date}</span>
          </div>
        ))}
      </div>
        ...
        {HOUR_MARKS.map((hour) => {
          ...
          return (
            <div key={hour} className="absolute inset-x-0" style={{ top: `${top}%` }}>
              <span className="tabular absolute -top-[8px] left-4 text-[11px] text-ui-muted">
          ...
          return (
            <div key={day} className="px-4 py-4">
              <p className="text-[13px]">
                ...
                <span className="tabular text-ui-muted">{DAY_LABELS[day].date}</span>
              </p>
    ...
    <div className="flex min-h-[52px] flex-wrap items-center gap-x-6 gap-y-2 border-t border-ui-border px-4 py-3 sm:px-6">
        ...
        <AnimatePresence initial={false}>
          {changeSummaries.slice(0, applied).map((c) => (
            ...
            >
              ...
              <span className="tabular text-ui-muted">{c.detail}</span>
            </motion.li>
          ))}
        </AnimatePresence>
      </ul>
      {updated && (
        <motion.span
          ...
          className="ml-auto text-[12px] text-ui-muted"
        >
          Sleep target kept · 10:30 pm
        </motion.span>
      )}
    </div>
  );
