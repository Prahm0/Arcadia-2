...
progress.get("/", async (c) => {
  ...
  const [events, sessions, totals, recoveredToday] = await Promise.all([
    ...
    database.select({ id: schema.xpEvents.id }).from(schema.xpEvents).where(and(eq(schema.xpEvents.userId, userId), eq(schema.xpEvents.source, "recovery"), gte(schema.xpEvents.createdAt, start))).limit(1),
  ]);
  // No study planned today is a rest day: there's no ring to close, rather
  // than a ring with a 1-minute goal.
  const restDay = events.length === 0;
  const goalMinutes = restDay ? 0 : Math.max(1, Math.round(events.reduce((sum, event) => sum + (event.endAt - event.startAt), 0) / MINUTE));
  ...
  const closed = !restDay && doneMinutes >= Math.ceil(goalMinutes * CONSISTENCY_THRESHOLD);
  // Using Life happened and still studying keeps the day on the streak.
  const protectedToday = recoveredToday.length > 0 && doneMinutes > 0;
  ...
  return c.json({ xp, level: level.level, title: level.title, levelProgress: { current: level.progress, needed: level.needed }, todayRing: { doneMinutes, goalMinutes, closed, restDay }, streak: { days: streaks.currentStreak, freezes: earnedFreezes, protectedToday }, achievements, personalBests: { longestFocus: Math.max(0, ...allSessions.map((session) => Math.round(session.seconds / 60))), longestStreak: streaks.longestStreak }, recentUnlocks: achievements.filter((achievement) => achievement.unlockedAt && achievement.unlockedAt >= now - DAY) });
});
